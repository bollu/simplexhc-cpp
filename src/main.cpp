...
class BuildCtx {
   public:
    ...
    BuildCtx(Module &m, StgIRBuilder &builder) {
        ...
        printInt =
                [&] {
                    ...
                    Function *printOnlyInt = getOrCreateFunction(m, FunctionType::get(builder.getVoidTy(), {builder.getInt64Ty()}, false), "printOnlyInt");
                    ...
                    BasicBlock *next = BasicBlock::Create(m.getContext(), "callnextfn", F);
                    ...
                    builder.SetInsertPoint(entry);
                    Value *returnTop = builder.CreateLoad(this->stackReturnContTop, "nReturnFrames");
                    Value *haveReturnFrames = builder.CreateICmpUGT(returnTop, builder.getInt64(1), "haveReturnFrames");
    ...
    ~BuildCtx() {
        ...
        Optional<LLVMClosureData> getTopLevelBindingFromName(std::string name)
            const {
        ...
        const StgType *getTypeFromRawType(const TypeRaw *tyraw) const {
            switch (tyraw->getKind()) {
                ...
                case TypeRaw::TK_Function: {
                    ...
                    const StgType *retty =
                        getTypeFromName(ft->getReturnTypeName());
       ...
       private:
        ...
        static void populateIntrinsicTypes(
            Module & m, StgIRBuilder & builder, TypeMapTy & typemap,
            DataConstructorMap & consmap, StgDataType * &primIntTy,
            StgDataType * &boxedTy) {
            // primInt
        ...
        static void addStack(
            Module & m, StgIRBuilder & builder, Type * elemTy, std::string name,
            size_t size, AssertingVH<Function> & pushFn,
        ...
        static void addPushToModule(Module & m, StgIRBuilder & builder,
            ...
            for (Argument &arg : F->args()) {
                ...
                Value *stackSlot = builder.CreateGEP(
                    stack, {builder.getInt64(0), idx}, "slot");
        ...
        static void addPopToModule(Module & m, StgIRBuilder & builder,
                                   Function * F, Value * stackTop,
                                   Value * stack) {
            assert(F);
            assert(stackTop);
        ...
        static Function *addEnterDynamicClosureToModule(
        ...
        static LLVMClosureData *addPrimMultiplyToModule(
            Module & m, StgIRBuilder builder, BuildCtx & bctx) {
            Function *F =
                createNewFunction(m,
                                  FunctionType::get(builder.getVoidTy(), {},
        ...
        LLVMClosureData createPrimFunction(
            ...
            identifiermap.insert(fnName,
                                 LLVMValueData(data.closure, functionType));
            return data;
        }
    };
    ...
    Value *materializeAtom(const Atom *a, StgIRBuilder &builder,
                           BuildCtx &bctx) {
        switch (a->getKind()) {
            case Atom::AK_Int:
    ...
    void materializeEnterDynamicClosure(Value *V, Module &m,
                                        StgIRBuilder &builder, BuildCtx &bctx) {
        errs() << "V : " << *V << "\n\n";
    ...
    void materializeConstructor(const ExpressionConstructor *c, Module &m,
        ...
        std::tie(cons, structType) =
            bctx.getDataConstructorFromName(c->getName());

        const int TotalSize =
            m.getDataLayout().getTypeAllocSize(structType);  // for the tag.

        Value *rawMem = builder.CreateCall(
            bctx.malloc, {builder.getInt64(TotalSize)}, "rawmem");
        Value *typedMem = builder.CreateBitCast(
            rawMem, structType->getPointerTo(), "typedmem");
        ...
        for (Atom *a : c->args_range()) {
            ...
            std::vector<Value *> idxs = {builder.getInt64(0),
                                         builder.getInt32(i)};
    ...
    void materializeCaseConstructorAltDestructure(const ExpressionCase *c,
        ...
        Value *StructPtr = builder.CreateBitCast(
            rawmem, DeclTy->getPointerTo(), DeclTy->getName() + "_structptr");
        ...
        for (Identifier var : d->variables_range()) {
            ...
            Value *Slot = builder.CreateGEP(StructPtr, Idxs,
                                            "slot_int_" + std::to_string(i));

            if (bctx.getTypeFromName(cons->getTypeName(i)) ==
                bctx.getPrimIntTy()) {
                Value *V =
                    builder.CreateLoad(Slot, "cons_" + std::to_string(i));
                bctx.insertIdentifier(var,
                                      LLVMValueData(V, bctx.getPrimIntTy()));
            } else {
    ...
    static const StgType *getCommonDataTypeFromAlts(const ExpressionCase *c,
        ...
        for (CaseAlt *a : c->alts_range()) {
            if (CaseAltDestructure *destructure =
                    dyn_cast<CaseAltDestructure>(a)) {
                const DataConstructor *dc =
                    std::get<0>(bctx.getDataConstructorFromName(
                        destructure->getConstructorName()));
                setCommonType(
                    bctx.getTypeFromName(dc->getParent()->getTypeName()));
    ...
    Function *materializeCaseConstructorReturnFrame(
        ...
        if (freeVarsInAlts.size() > 0) {
            Value *closureAddr = builder.CreateLoad(bctx.enteringClosureAddr,
                                                    "closure_addr_int");
        ...
        if (c->alts_size() == 1 && isa<CaseAltDefault>(*(c->alts_begin()))) {
            ...
            const CaseAltDefault *default_ =
                cast<CaseAltDefault>(*c->alts_begin());
            ...
            switch (a->getKind()) {
                case CaseAlt::CAK_Destructure: {
                    ...
                    BasicBlock *bb = BasicBlock::Create(
                        m.getContext(), d->getConstructorName(), f);
                    ...
                    const DataType *dataType =
                        cast<StgDataType>(
                            ...
                            ->getDataType();
                    const int Tag = dataType->getIndexForConstructor(
                        std::get<0>(bctx.getDataConstructorFromName(
                ...
                case CaseAlt::CAK_Int:
                    assert(false &&
                           "case of a non-int scrutinee cannot have int");
                    break;
                case CaseAlt::CAK_Variable: {
                    CaseAltVariable *altVariable = cast<CaseAltVariable>(a);
                    BasicBlock *bb = BasicBlock::Create(
                        m.getContext(), altVariable->getLHS(), f);
                    builder.SetInsertPoint(bb);
                    ...
                    Value *raw = builder.CreateCall(
                        bctx.popInt, {}, altVariable->getLHS() + "_raw");
                    const StgType *ty =
                ...
                case CaseAlt::CAK_Default:
                    assert(false &&
                           "unimplemented alt codegen for cak_default");
            }
        }
        return f;
    }
    ...
    Function *materializePrimitiveCaseReturnFrame(
        ...
        if (const CaseAltDefault *cd = c->getDefaultAlt()) {
            ...
            // the alt.
            bctx.insertIdentifier(
                cv->getLHS(), LLVMValueData(scrutinee, bctx.getPrimIntTy()));
    ...
    std::set<Identifier> getFreeVarsInExpression(
        ...
        switch (e->getKind()) {
            ...
            case Expression::EK_Cons: {
                const ExpressionConstructor *cons =
                    cast<ExpressionConstructor>(e);
            ...
            case Expression::EK_Case: {
                ...
                const std::set<Identifier> scrutineeFree(
                    getFreeVarsInExpression(c->getScrutinee(),
                                            staticBindingsRange));
                ...
                for (const CaseAlt *alt : c->alts_range()) {
                    ...
                    const std::set<Identifier> rhsFree =
                        getFreeVarsInExpression(alt->getRHS(),
                                                staticBindingsRange);

                    std::set_difference(
                        rhsFree.begin(), rhsFree.end(), lhsBound.begin(),
                        lhsBound.end(),
    ...
    std::set<Identifier> getIdentifiersInAltLHS(const CaseAlt *alt) {
        switch (alt->getKind()) {
            ...
            case CaseAlt::CAK_Destructure: {
                ...
                for (Identifier id : destructure->variables_range())
                    ids.insert(id);
                return ids;
            }
        }
    ...
    void materializeCase(const ExpressionCase *c, Module &m,
                         StgIRBuilder &builder, BuildCtx &bctx) {
        ...
        Function *continuation = [&] {
            if (bctx.getPrimIntTy() == scrutineety) {
                ...
                return materializeCaseConstructorReturnFrame(c, freeVarsInAlts,
                                                             m, builder, bctx);
            }
        }();
    ...
    Value *_allocateLetBindingDynamicClosure(const Binding *b, BasicBlock *BB,
        ...
        const uint64_t sizeInBytes =
            m.getDataLayout().getTypeAllocSize(closureTy);
        Value *rawMem = builder.CreateCall(
            bctx.malloc, {builder.getInt64(sizeInBytes)}, "rawmem");
        Value *typedMem = builder.CreateBitCast(
            rawMem, closureTy->getPointerTo(), "closure_" + b->getName());
    ...
    StgFunctionType *createStgTypeForLambda(const Lambda *l,
                                            const BuildCtx &bctx) {
    ...
    void materializeLet(const ExpressionLet *l, Module &m,
                        StgIRBuilder &builder, BuildCtx &bctx) {
        ...
        for (Binding *b : l->bindings_range()) {
            ...
            const StgType *bindingty =
                createStgTypeForLambda(b->getRhs(), bctx);
        ...
        for (Binding *b : l->bindings_range()) {
            ...
            Value *fnSlot = builder.CreateGEP(
                cls, {builder.getInt64(0), builder.getInt32(0)},
                b->getName() + "_fn_slot");
    ...
    void materializeEnterInt(Value *v, std::string contName, Module &m,
                             StgIRBuilder &builder, BuildCtx &bctx) {
        builder.CreateCall(bctx.pushInt, {v});
        Value *cont = builder.CreateCall(bctx.popReturnCont, {},
                                         "return_cont_" + contName);
    ...
    void materializeExpr(const Expression *e, Module &m, StgIRBuilder &builder,
        ...
        switch (e->getKind()) {
            case Expression::EK_Ap:
                ...
                break;
            case Expression::EK_Cons:
                materializeConstructor(cast<ExpressionConstructor>(e), m,
                                       builder, bctx);
                break;
            case Expression::EK_Case:
            ...
            case Expression::EK_Let:
                ...
                break;
            case Expression::EK_IntLiteral:
                materializeExprIntLiteral(cast<ExpressionIntLiteral>(e), m,
                                          builder, bctx);
        };
    }
    ...
    void materializeLambda(const Lambda *l, Module &m, StgIRBuilder &builder,
        ...
        for (const Parameter *p : l->bound_params_range()) {
            const StgType *Ty = bctx.getTypeFromRawType(p->getTypeRaw());
            if (Ty == bctx.getPrimIntTy()) {
                Value *pv = builder.CreateCall(bctx.popInt, {},
                                               "param_" + p->getName());
                bctx.insertIdentifier(p->getName(),
                                      LLVMValueData(pv, bctx.getPrimIntTy()));
            } else {
                Value *pv = builder.CreateCall(bctx.popBoxed, {},
                                               "param_" + p->getName());
    ...
    LLVMClosureData materializeTopLevelStaticBinding(const Binding *b,
                                                     Module &m,
                                                     StgIRBuilder &builder,
                                                     BuildCtx &bctx) {
